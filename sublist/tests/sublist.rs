...
#[bench]
fn superlist_at_start(b: &mut Bencher) {
    // assert_eq!(Comparison::Superlist, sublist(&[1, 2, 3, 4, 5], &[1, 2, 3]));
    b.iter(|| sublist(&[1, 2, 3, 4, 5], &[1, 2, 3]));
}
...
#[test]
#[ignore]
fn numbers_in_diferent_places() {
    assert_ne!(
        Comparison::Sublist,
        sublist(&[1, 3, 5], &[1, 2, 3, 4, 5, 6, 7, 8, 9])
    );
}
#[test]
#[ignore]
fn numbers_in_diferent_placess() {
    assert_ne!(
